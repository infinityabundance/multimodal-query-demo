...
use mqd_engine::query::{latest_at, range_scan, LatestAtQuery, QueryPlan, RangeScanQuery};
use mqd_engine::store::DEFAULT_BUCKET_NS;
use mqd_engine::{ingest::jsonl_events, ingest_stream, IngestConfig, Store};
use tokio_util::sync::CancellationToken;

use crate::cli::{LatestAtArgs, QueryCommand, RangeArgs};
...
pub async fn run(
    ...
    match cmd {
        QueryCommand::LatestAt(args) => run_latest_at(args, reporter, json_mode).await,
        QueryCommand::Range(args) => run_range(args, reporter, json_mode).await,
    }
}

async fn run_latest_at(
    args: LatestAtArgs,
    reporter: Arc<dyn StatusReporter>,
    json_mode: bool,
) -> Result<()> {
    ...
    reporter.phase("query latest-at");
    let kind = args.kind.to_kind();
    let entity = args
        .entity
        .as_ref()
        .map(|e| EntityPath::new(e.as_str()))
        .transpose()
        .context("parsing --entity")?;
...
fn emit_result(
    ...
    if batch.num_rows() == 0 {
        ...
        // Newline so the table is visually separated from the status stream.
        println!("\n{pretty}");
    }
...
fn emit_rows_ndjson(batch: &RecordBatch) -> Result<()> {
    ...
    // LineDelimitedWriter emits one JSON object per row, newline-terminated. This lets the
    // caller pipe the result through `jq` or `grep` cleanly.
    let mut writer = arrow::json::writer::LineDelimitedWriter::new(&mut locked);
    writer.write(batch)?;
    writer.finish()?;
    // Final flush + line break so the result stream terminates cleanly.
    let _ = schema;
    locked.write_all(b"")?;
    Ok(())
}
